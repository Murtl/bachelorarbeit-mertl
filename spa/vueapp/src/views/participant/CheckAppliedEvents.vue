...
onBeforeMount(async () => {
  appliedEvents.value = await eventStore.getEventsOfParticipant(
    userStore.userName,
  );
});

/**
 * @description entfernt den angemeldeten Benutzer von einem Event
 * @param eventId die ID des Events, für den der Teilnehmer entfernt werden soll
 */
...
<template>
  ...
  <section
    class="main-section"
    v-for="(event, index) in appliedEvents"
    :key="index"
  >
    <EventSegment
      :id="event.id"
